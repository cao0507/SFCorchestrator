...
class configure_file(object):
    ...
    def configure_vnfd(self, vnf, sfc_mapper):
        ...
        # update VNFD according to vnf and sfc_mapper
        vnfd_template["vnfd"]["tenant_id"] = self.platform.get_tenant_id()
        vnfd_template["vnfd"]["name"] = \
            sfc_mapper["name"] + "_" + vnf['name'] + "_Description"
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]
        
        ["properties"]["num_cpus"] = vnf["flavor"]["cpu"]
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]
        
        ["properties"]["mem_size"] = str(vnf["flavor"]["memory"]) + " MB"
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]
        
        ["properties"]["disk_size"] = str(vnf["flavor"]["disk"]) + " GB"
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["properties"]["image"] = vnf["type"]
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["properties"]["user_data"] = vnf["rule"]
        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]
        
        ["node_templates"]["VDU1"]["properties"]["availability_zone"] \
            = sfc_mapper["mapper"][vnf["name"]]

        # write VNFD into file
        vnfd_file = \
            sfc_mapper["name"] + "_" + vnf['name'] + "_Description.json"
        f = open("vnfd/" + vnfd_file, 'w')
        data = json.dumps(vnfd_template,
    ...
    def configure_vnffgd(self, sfc_orchestrator):
        ...
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        
        ["node_templates"]["Forwarding_path1"]["properties"]["policy"]
        
        ["criteria"][0]["network_src_port_id"] =\
            self.platform.get_src_port_id()
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        
        ["node_templates"]["Forwarding_path1"]["properties"]["policy"]
        
        ["criteria"][0]["ip_dst_prefix"] = self.platform.get_dst_ip()
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        
        ["node_templates"]["Forwarding_path1"]["properties"]["path"] = path
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        ...
        ["groups"]["VNFFG1"]["properties"]["connection_point"] = \
            connection_point
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        ...
        ["groups"]["VNFFG1"]["properties"]["dependent_virtual_link"] = \
            dependent_virtual_link
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]
        
        ["groups"]["VNFFG1"]["properties"]["number_of_endpoints"] = \
