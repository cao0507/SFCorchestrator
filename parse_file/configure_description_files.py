...
class configure_file(object):
    ...
    def configure_vnfd(self, vnf, sfc_mapper):
        ...
        vnfd_template["vnfd"]["tenant_id"] \
                = config.tenant_id

        vnfd_template["vnfd"]["name"] \
                = sfc_mapper["name"] + "_" +  vnf['name'] + "_Description"

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]["properties"]["num_cpus"] \
                = vnf["flavor"]["cpu"]

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]["properties"]["mem_size"] \
                = str(vnf["flavor"]["memory"]) + " MB"

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["capabilities"]["nfv_compute"]["properties"]["disk_size"] \
                = str(vnf["flavor"]["disk"]) + " GB"

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["properties"]["image"] \
                = vnf["type"]

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["properties"]["user_data"] \
                = user_data

        vnfd_template["vnfd"]["attributes"]["vnfd"]["topology_template"]["node_templates"]["VDU1"]["properties"]["availability_zone"] \
                = sfc_mapper["mapper"][vnf["name"]]
    ...
    def configure_vnffgd(self, sfc_orchestrator):
        ...
        for src_port_id in config.src_port_list:
            classifier["network_src_port_id"] = src_port_id
            classifier["ip_dst_prefix"] = config.dst_ip
            criteria.append(deepcopy(classifier))
        ...
        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["node_templates"]["Forwarding_path1"]["properties"]["policy"]["criteria"] \
                = criteria

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["node_templates"]["Forwarding_path1"]["properties"]["path"] \
                = path

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["node_templates"]["Forwarding_path1"]["properties"]["id"] \
                = random.randint(1, 100)

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["groups"]["VNFFG1"]["properties"]["connection_point"] \
                = connection_point

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["groups"]["VNFFG1"]["properties"]["constituent_vnfs"] \
                = constituent_vnfs

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["groups"]["VNFFG1"]["properties"]["dependent_virtual_link"] \
                = dependent_virtual_link

        vnffgd_template["vnffgd"]["template"]["vnffgd"]["topology_template"]["groups"]["VNFFG1"]["properties"]["number_of_endpoints"] \
                = len(sfc_orchestrator["chain"])       

        # write VNNFGD into file
        data = json.dumps(vnffgd_template,
                          sort_keys=False, indent=4, separators=(',', ': '))
        vnffgd_file = sfc_orchestrator["name"] + "_vnffg_Description.json"
