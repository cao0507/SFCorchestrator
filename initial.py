...
import config.abs_dir as abs_dir
...
def initial_platform():
    ...
    f = open(abs_dir + "openstack_platform/auth.json", 'r')
    auth = json.load(f)
    f.close()
    ...
    f = open(abs_dir + "openstack_platform/vim.json", 'w')
    ...
    if default_vim is None:
        tacker_vim.register_vim(abs_dir + "openstack_platform/vim.json") 
